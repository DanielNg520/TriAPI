...
- **Secrets use sops + age, not `.env`** — `config/secrets.enc.yaml` is sops-encrypted ciphertext, safe to commit; only `scripts/secrets_loader.py` (which shells out to `sops -d`) ever sees plaintext values, at runtime, in memory.
