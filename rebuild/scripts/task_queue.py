...
def cmd_approve(args: argparse.Namespace, conn: sqlite3.Connection) -> int:
    ...
    status = row["status"]
    if status != "pending":
        print(f"task {args.id} is {status}, expected pending", file=sys.stderr)
        return 1
    ...
    insert_event(conn, args.id, "approved", None, ts)
    conn.commit()
    return 0
...
def cmd_claim(args: argparse.Namespace, conn: sqlite3.Connection) -> int:
    ...
    if args.id is not None:
        row = conn.execute(
            "SELECT id, status, worker, description, depends_on FROM tasks WHERE id = ?",
            (args.id,),
        ).fetchone()
        if row is None:
            print(f"task {args.id} not found", file=sys.stderr)
        ...
        if row["status"] != "approved":
            print(f"task {args.id} is {row['status']}, expected approved", file=sys.stderr)
            return 1
        ...
        if depends_on is not None:
            ...
            if dep_row is None or dep_row["status"] != "done":
                print(
                    f"task {args.id} depends on {depends_on} which is not done",
                    file=sys.stderr,
                )
                return 1
        ...
        if row is None:
            print("no approved task ready to claim", file=sys.stderr)
            return 1
        task_id = row["id"]
        worker = row["worker"]
        description = row["description"]
    ...
    ts = utcnow()
    cur = conn.execute(
        "UPDATE tasks SET status = 'in_progress', updated_at = ? WHERE id = ? AND status = 'approved'",
        (ts, task_id),
    )
    if cur.rowcount != 1:
        conn.rollback()
        print(f"task {task_id} was already claimed", file=sys.stderr)
        return 1

    insert_event(conn, task_id, "claimed", None, ts)
    conn.commit()
    print(f"{task_id}\tin_progress\t{worker or ''}\t{description}")
    return 0
...
def cmd_complete(args: argparse.Namespace, conn: sqlite3.Connection) -> int:
    ...
    if status != "in_progress":
        print(f"task {args.id} is {status}, expected in_progress", file=sys.stderr)
        return 1

    ts = utcnow()
    conn.execute(
        "UPDATE tasks SET status = ?, result = ?, updated_at = ? WHERE id = ?",
        (args.status, args.result, ts, args.id),
    )
    insert_event(
        conn,
        args.id,
