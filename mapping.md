...
- `dispatcher.py` — Tier 2 as manager (distinct from `tier2_escalate.py`'s repair role): `breakdown_plan()` converts an approved plan into strict JSON (`{"phases": [{"name", "items": [ITEM]}]}`, where ITEM is either a file item `{"description", "target", "build_cmd"}` or a git item `{"description", "git": {"action", ...}}`) via Gemini's JSON mode; `dispatch()` walks items sequentially, routing file items to `orchestrator.run_task()` and git items to `git_ops` via `_dispatch_git_item()`, persisting `logs/runs/<run_id>.json` after every item, stopping on the first non-success item (retried, not skipped, on resume). Empty `build_cmd` (a non-code file item) becomes `test -f <target>`, not the code-project default build.
- `git_ops.py` — `clone(url, path)`, `pull(repo_dir)`, `push(repo_dir, message, branch=None)`. Direct shell git commands, logged via `tri_logging`. Safety rails: never force-push; `push()` never lands directly on `main`/`master` unless a plan step explicitly names that branch — otherwise creates a new `triapi/<dirname>-<timestamp>` branch. Never handles credentials itself — relies on the machine's existing git credential setup.
- `triapi.py` — CLI entry point (installed as the real `triapi` command via `~/.local/bin/triapi` symlink). Subcommands: `plan` (interactive, foreground-only), `dispatch [--background]` (can run detached, safe against SSH/Tailscale disconnects), `status`, `list`.
