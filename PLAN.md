...
6. **HTTPS git remotes are not supported for push/pull in this environment** — confirmed in Phase 9.1: there's no non-interactive HTTPS credential helper configured, and the previously-configured GUI askpass hangs indefinitely rather than failing when invoked from a headless/background context. `git_ops.py` now fails fast with a clear error in this case rather than hanging, but the actual fix for a real push is to use an SSH remote (`git@github.com:...`), which is confirmed working. Worth deciding later whether `git_ops.clone()` should auto-rewrite an `https://github.com/...` URL to SSH, or just keep surfacing the fast, clear failure and let the plan/user specify SSH.
