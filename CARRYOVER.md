...
| File | Status |
|---|---|
| [`docs/carryover/20260829-000000-fact-proposal-port-plus-severe-daemon-blocker-found.md`](docs/carryover/20260829-000000-fact-proposal-port-plus-severe-daemon-blocker-found.md) | **ACTIVE** — resumed oh-my-llama's Phase 5G; found+fixed a real feature regression (fact-proposal/provenance safety gate never ported to `src/semai`), ported it properly across 4 supervised dispatch runs, fully verified (176 pytest passed, 4 subtests, all script suites green). Re-attempting the actual Phase 5G file deletions then surfaced a *second, more severe* blocker: the new daemon still constructs and depends on a real, live `ohmyllama.orchestrator.AsyncOrchestrator` for config/concurrency/DLQ/model-warming/memory-consolidation/reminders — deleting it would break the running daemon. Neither Phase 5G nor the user-requested full `ohmyllama`→`SemAI` rename (Phase 7) was attempted as a result; nothing deleted or renamed, no reboot performed. Also fixed on the TriAPI side: a `dispatcher.py` bug where whole-file-deletion plan items inconsistently bypassed (or didn't) the LLM edit-block path depending on the planner's `verify_only` guess, now enforced correctly (`99df062`); a stale AGENTS.md claim (`8024aa6`). **Session closed here, nothing mid-flight; next action needs a human decision** — full detail in oh-my-llama's own `docs/Agent/CARRYOVER.md` |
