...
| File | Status |
|---|---|
| [`docs/carryover/20260828-090500-agy-argv-fix-tier1-planner-moved-off-openrouter.md`](docs/carryover/20260828-090500-agy-argv-fix-tier1-planner-moved-off-openrouter.md) | **ACTIVE** — `agy` argv-length crash fixed+committed (`3cbdeba`); `tier_1_planner` moved off two hallucinating free OpenRouter models onto `agy`/Gemini 3.1 Pro (reused from `tier_3_debugger`), `dots-3-note-preview` demoted into `tier_3_debugger` as a documented-inert secondary; effort-forwarding bug and librarian `fallback_openrouter` endpoint bug found+fixed along the way; full suite 219 tests OK; a new multi-turn planner-statelessness bug found, still open, queued top priority; working tree clean, nothing mid-flight |
