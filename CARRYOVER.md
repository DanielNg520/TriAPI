...
| File | Status |
|---|---|
| [`docs/carryover/20260828-105000-week-audit-plus-live-scope-guard-catch.md`](docs/carryover/20260828-105000-week-audit-plus-live-scope-guard-catch.md) | **ACTIVE** — full-week audit (2026-08-23–28, ~55 commits) found zero functional regressions, only 5 minor doc-accuracy issues; fixing 2 of them via supervised `triapi plan`/`dispatch` caught Tier 4 deleting `_is_deepseek_peak_hours()` out-of-scope (`db5fe8b`, broke 4 tests), reverted (`7c21561`); root-caused and fixed the `scope_guard.py` blind spot that missed it — whole-function deletions now caught via hunk-body scanning (`5b40110`); full suite 244 tests OK; working tree clean, nothing mid-flight |
